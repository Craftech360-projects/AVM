...
class MessageProvider {
  ...
  MessageProvider._internal() : _box = ObjectBoxUtil.instance.store.box<Message>();

  // Constructor for Workmanager isolate
  MessageProvider.withStore(Store store) : _box = store.box<Message>();
  ...
  Future<List<Message>> retrieveMostRecentMessages({
    ...
    Query<Message> query = _box
        .query(Message_.fromIntegration.equals(false))
        .order(Message_.createdAt, flags: Order.descending)
        .build();
    query.limit = limit;
    var messages = query.find();
