...
class _LocalSyncState extends State<LocalSync> {
  ...
  Future<void> _sendAllFiles() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var file in _files) {
          try {
            await file.delete();
            if (mounted) {
              avmSnackBar(context, 'Deleted file: ${file.path}');
            }
            // print('Deleted file: ${file.path}');
          } on Exception catch (e) {
            if (mounted) {
              avmSnackBar(context, 'Error deleting file: ${file.path} - $e');
            }
          }
        }
      ...
      if (mounted) {
        avmSnackBar(context, "Oops! Something went wrong. Please try again.");
      }
    } finally {
      setState(() => _loading = false);
    }
  ...
  Future<Map<String, dynamic>> checkTaskStatus(String taskId) async {
    ...
    // var uri = Uri.parse('https://living-alien-polite.ngrok-free.app/v1/status/$taskId');
  ...
  dynamic Function(Message, Memory?)? get sendMessageToChat => null;
      ...
      for (var taskId in taskIdsCopy) {
        try {
          ...
          if (status['status'] == 'completed') {
            if (mounted) {
              avmSnackBar(context, 'Task $taskId completed');
            }
            // Remove the taskId from the original list outside of the loop
            taskIds.remove(taskId);
          } else if (status['status'] == 'failed') {
            if (mounted) {
              avmSnackBar(context, 'Task $taskId failed: ${status['error']}');
            }
            // Remove the taskId from the original list outside of the loop
            taskIds.remove(taskId);
          }
          ...
          if (mounted) {
            avmSnackBar(context, 'Error checking task status: $e');
          }
        }
          ...
          onPressed: () {
            showDialog(
              ...
              builder: (context) => AlertDialog(
                ...
                content:
                    const Text('Are you sure you want to delete all files?'),
                actions: [
                  TextButton(
