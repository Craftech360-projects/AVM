...
Future<bool> assignConversationTranscriptSegment(
  ...
  var response = await makeApiCall(
    url:
        '${Env.apiBaseUrl}v1/memories/$conversationId/segments/$segmentIdx/assign?value=${isUser ?? personId}'
...
Future<bool> setConversationVisibility(String conversationId,
  ...
  var response = await makeApiCall(
    url:
        '${Env.apiBaseUrl}v1/memories/$conversationId/visibility?value=$visibility&visibility=$visibility',
    headers: {},
    method: 'PATCH',
    body: '',
  );
...
class SyncLocalFilesResponse {
  ...
  factory SyncLocalFilesResponse.fromJson(Map<String, dynamic> json) {
    return SyncLocalFilesResponse(
      newConversationIds: ((json['new_memories'] ?? []) as List<dynamic>)
          .map((val) => val.toString())
          .toList(),
      updatedConversationIds:
          ((json['updated_memories'] ?? []) as List<dynamic>)
              .map((val) => val.toString())
              .toList(),
    );
  }
