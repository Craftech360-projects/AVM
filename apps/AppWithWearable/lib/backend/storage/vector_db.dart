...
import 'package:avm/backend/storage/dvdb/dvdb_helper.dart';
import 'package:avm/backend/storage/memories.dart';

var collection = DVDB().collection("memories");

Future<void> storeMemoryVector(
    MemoryRecord memory, List<double> embedding) async {
  collection.addDocument(
      memory.id, memory.getStructuredString(), Float64List.fromList(embedding));
}
