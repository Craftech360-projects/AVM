...
Future<List<dynamic>> retrieveRAGContext(String message,
  ...
  debugPrint('retrieveRAGContext>>>>>>>>>>>>: $prevMessages');
  ...
  var startTimestamp = datesRange.isNotEmpty
      ? datesRange[0].millisecondsSinceEpoch ~/ 1000
      : null;
  var endTimestamp = datesRange.isNotEmpty
      ? datesRange[1].millisecondsSinceEpoch ~/ 1000
      : null;

  // TODO: how to show all the memories used in the chat, maybe a expand toggle?
  Future<List<List<String>>> memoriesByTopic =
      _retrieveMemoriesByTopic(topics, startTimestamp, endTimestamp, message);
  List<Memory> memories = [];
  if (topics.isNotEmpty) {
    List<List<String>> memoriesIdList = await memoriesByTopic;
    List<String> memoriesId = memoriesIdList
        .reduce((value, element) => value + element)
        .toSet()
        .toList();
    debugPrint(
        'queryPineconeVectors memories from topics: ${memoriesId.length}');
